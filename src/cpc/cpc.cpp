...
int Cpc::init(Prefs* prefs)
{
	...
	int ret = mMemman.init(mPrefs->getNum("ramsize"), mPrefs->getPath("cpcrom"), mPrefs->getPath("amsdos"));
	if (ret)
	{
		if (ret == MemMan::ErrRamSize) cerr << "Incorrect RAM-size.\n";
		if (ret == MemMan::ErrMemory)  cerr << "Not enough Memory.\n";
		if (ret &  MemMan::ErrCpcRom)  cerr << "Could not read CPC-ROM file.\n";
		if (ret &  MemMan::ErrAmsdos)  cerr << "Could not read AMSDOS-ROM file.\n";
		return ret;
	}
...
UBYTE Cpc::z80_in_handler(REGPAIR port)
{
	...
	if (!(port.b.h & 0x40)) // CRTC Read selected register
	{
		if ((port.b.h & 3) == 3)
		{
			if ((mCrtc.selected() > 11) && (mCrtc.selected() < 18)) retval=mCrtc.read(); // valid range?
			else retval=0; // write only registers retval=0
		}
	}
	// **********************************************************************
	// *** PPI
				...
				if (mPpi.control() & 0x10) // port A set to input?
				{
					if ((mPsg.control() & 0xc0) == 0x40) // PSG control set to read?
					{
						if (mPsg.selected() < 16) // within valid range?
						{
							...
							{
								if (!(mPsg.registerAY(7) & 0x40)) retval=mKeyboard.value(mKeyboard.row() & 0x0f); // port A in input mode? read mKeyboard matrix node status
								else retval=mPsg.registerAY(14) & mKeyboard.value(mKeyboard.row() & 0x0f); // retval=last value w/ logic AND of input
							}
							else if (mPsg.selected() == 15) // PSG port B?
							{
								if ((mPsg.registerAY(7) & 0x80)) retval=mPsg.registerAY(15); // port B in output mode?retval=stored value
							}
							else retval=mPsg.registerAY(); // read PSG register
						}
					}
				}
				else retval=mPpi.portA();
				break;

				case 1: // PPI PortB
					if (mPpi.control() & 2) retval=(mPpi.jumpers() | (mCrtc.flags() & Crtc::VS));
					else retval=mPpi.portB();
					...
					case 2: // PPI PortC
					{
						...
						if (direction)        // either half set to input?
						{
							if (direction & 8) // upper half set to input?
							{
								...
								UBYTE value = mPpi.portC() & 0xc0; // isolate PSG control bits
							...
							//if (CPC.tape_motor) retval |= 0x10;    // set the bit if the tape motor is running
							}
...
void Cpc::z80_out_handler(REGPAIR port, UBYTE value)
{
	...
	if (!(port.b.h & 0x40)) // CRTC select?
	{
		...
		if (sel == 0) mCrtc.select(value); // CRTC register select?
		else
			if (sel == 1) // CRTC write data?
				if (mCrtc.selected() < 16) mCrtc.write(value); // only registers 0 - 15 can be written to
	}
			...
			case 0: // write to port A?
				...
				case 1: // write to port B?
					...
					case 2: // write to port C?
						...
						if (!(mPpi.control() & 1)) mKeyboard.setRow(value & 0x0F); // output lower half? @todo check 0x0f
						...
						case 3: // modify PPI control
							...
							else // bit manipulation of port C data
							{
								if (value & 1) // set bit?
								{
									...
									mPpi.setC(mPpi.portC() | bit_values[bit]); // set requested bit
							...
							//CPC.tape_motor = PPI.portC & 0x10;
								...
								else
								{
									...
									mPpi.setC(mPpi.portC() & ~(bit_values[bit])); // reset requested bit
									if (!(mPpi.control() & 1)) // output lower half?
									{
							//CPC.mKeyboard_line = PPI.portC;
										mKeyboard.setValue(mPpi.portC());
	...
	if (((port.b.h == 0xFA) && (!(port.b.l & 0x80)))) // floppy motor control?
	{
		...
		if ((port.b.h == 0xFB) && (!(port.b.l & 0x80))) // FDC data register?
	{
...
void Cpc::waitstates()
{
	...
	if (mSound.enabled())
	{
		...
		{
			mSound.setCycleCountBoth(mSound.cycleCountBoth() - mSound.cycleCountInitBoth());
